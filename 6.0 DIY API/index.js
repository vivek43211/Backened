...
app.get("/jokes/:id",(req,res)=>{
  ...
  const findjoke = jokes.find((jokes)=>jokes.id ===id);
  res.json(findjoke)
})
// params is path variable 

//3. GET a jokes by filtering on the joke type
app.get("/filter",(req,res)=>{
  const type = req.query.type;
  const findtype = jokes.filter((jokes)=>jokes.jokeType ===type);
  res.json(findtype)
})
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  if(searchIndex > -1){
    ...
    res
      .status(404)
      .json({ error: `You are not authorised ${id} to perform this action.` });
  }
})
